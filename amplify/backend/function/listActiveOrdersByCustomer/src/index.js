...
exports.handler = async event => {
  ...
  if (!(await checkSecurity(args, identity))) {
    const resp = {
      statusCode: 401,
      body: 'Unauthorized',
    };
  ...
  const redisVal = await redisGet('ACTIVEORDERSBYCUSTOMER#' + args.customerId);
  ...
  const resp = await listActiveOrdersByCustomer(args.customerId);

  await redisSet('ACTIVEORDERSBYCUSTOMER#' + args.customerId, JSON.stringify(resp), 'EX', 60 * 60 * 24);
  console.log('SET REDIS');

  return resp;
};

const checkSecurity = async (args, identity) => {
